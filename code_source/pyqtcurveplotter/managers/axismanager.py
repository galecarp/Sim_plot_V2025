...
class AxisManager(AbstractManager):
    ...
    # 信号定义
    sig_axis_added = pyqtSignal(str)     # 轴添加时发射，参数为轴名称
    sig_axis_removed = pyqtSignal(str)   # 轴删除时发射，参数为轴名称
    sig_axis_changed = pyqtSignal(str)   # 轴配置变更时发射，参数为轴名称
    sig_axes_batch_added = pyqtSignal(list)    # 批量轴添加时发射，避免多次刷新
    sig_axes_batch_removed = pyqtSignal(list)  # 批量轴删除时发射，避免多次刷新
    sig_axes_batch_changed = pyqtSignal(list)  # 批量轴变更时发射，避免多次刷新
