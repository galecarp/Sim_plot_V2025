...
from code_source.pyqtcurveplotter.enums.valueenum import UnitValue
...
class AxisConfigPanel(QWidget):
    ...
    # 信号定义
    sig_config_changed: pyqtSignal = pyqtSignal()  # 配置改变信号
    sig_delete_requested: pyqtSignal = pyqtSignal(str)  # 删除请求信号，参数为轴名称
    ...
    def _init_layout_main(self):
        ...
        # === 4. 删除按钮（仅副轴显示）===
        if not self.axisconfig.bol_is_prim_axis:
            self.btn_delete = QPushButton(self.tr("删除此轴", 'f_delete_axis'))
    ...
    def _init_layout_basic_config(self,
        ...
        layout_main.addRow(
            self.tr("轴标签:", 'f_label_axis'), self.edit_label)
        ...
        layout_main.addRow(
            self.tr("单位:", 'f_unit_data'), combo_unit)
        ...
        layout_main.addRow(
            self.tr("轴颜色:", 'f_color_axis'), layout_color)
    ...
    def _create_range_groupbox(self) -> QGroupBox:
        ...
        """
        # 分组盒子
        groupbox_range = QGroupBox(
            self.tr("范围设置", 'f_setting_range'))
        boxlayout_range = QVBoxLayout()
        
        # 1.创建单选按钮：自动 vs 手动
        self.radiobtn_range_auto = QRadioButton(
            self.tr("自动", 'f_auto'))
        self.radiobtn_range_manual = QRadioButton(
            self.tr("手动", 'f_manual'))
        ...
        layout_manual.addRow(
            self.tr("最小值:", 'f_minimum'),
            self.spin_range_min)
        layout_manual.addRow(
            self.tr("最大值:", 'f_maximum'),
            self.spin_range_max)
    ...
    def _create_alignment_groupbox(self) -> QGroupBox:
        ...
        """
        # 分组盒子
        groupbox_align = QGroupBox(
            self.tr("对齐设置", 'f_setting_alignment'))
        boxlayout_align = QVBoxLayout()
        
        # 1.启用对齐复选框
        self.checkbox_align_enable = QCheckBox(
            self.tr("启用对齐", 'f_enable_alignment'))
        ...
        layout_align_to_axis.addRow(
            self.tr("对齐到轴:", 'f_align_to_axis'),
            self.combo_align_to_axis)
        ...
        layout_align_to_axis.addRow(
            self.tr("对齐模式:", 'f_mode_alignment'),
            self.combo_align_mode)
    ...
    def _select_color_axis_process(self):
        ...
        color = QColorDialog.getColor(
            ...
            parent=self,
            title=self.tr("选择轴颜色", 'f_select_color_axis')
        )
    ...
    def _create_all_alignment_param_widgets(self):
        ...
        # 添加到布局
        self.layout_align_params.addRow(
            self.tr("本轴值:", 'f_value_axis_src'),
            self.spin_align_value_src)
        self.layout_align_params.addRow(
            self.tr("目标轴值:", 'f_value_axis_tgt'),
            self.spin_align_value_tgt)
        self.layout_align_params.addRow(
            self.tr("对齐跨度的拉伸系数:", 'f_scale_span'),
            self.spin_scale_span)
