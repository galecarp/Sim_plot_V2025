...
class CurveManager(AbstractManager):
    ...
    # 信号定义
    sig_curve_added = Signal(str)     # 曲线添加时发射，参数为列名
    sig_curve_removed = Signal(str)   # 曲线删除时发射，参数为列名
    sig_curve_changed = Signal(str)   # 曲线配置变更时发射，参数为列名
    sig_curves_batch_added = Signal(list)    # 批量曲线添加时发射，避免多次刷新
    sig_curves_batch_removed = Signal(list)  # 批量曲线删除时发射，避免多次刷新
    sig_curves_batch_changed = Signal(list)  # 批量曲线变更时发射，避免多次刷新
